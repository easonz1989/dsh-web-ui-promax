...
const dict = {
  en: {
    ...
    uiEffects: 'UI Effects', original: 'Original UI', originalHint: 'Harness native appearance with no ProMax visual overrides.',
    ios: 'iOS', iosHint: 'Swift Glass motion with a Liquid Glass interface.',
  ...
  zh: {
    ...
    uiEffects: 'UI 特效', original: '原生界面', originalHint: '使用 Harness 原生外观，不应用 ProMax 视觉覆盖。',
    ios: 'iOS', iosHint: 'Swift Glass 动效与 Liquid Glass 界面。',
...
.promax-effect-cube{border-color:var(--dsw-alias-border-l2);background:transparent}.promax-effect-selected{border-color:var(--dsw-static-neutral-bluish-400);background:var(--dsw-alias-bg-module-platform)}.promax-effect-preview-original{background:linear-gradient(135deg,var(--dsw-alias-bg-layer-1),var(--dsw-alias-bg-module-platform));box-shadow:0 7px 18px rgba(0,0,0,.10);backdrop-filter:none;-webkit-backdrop-filter:none;animation:none}
...
function UiEffectsRow({ t, getEffect, subscribeEffect, setEffect }: EffectsProps) {
  ...
  const choose = async (next: UiEffect): Promise<void> => {
  ...
  return <div className="promax-effects-group">
    ...
    <div className="promax-effects-row">
      {([
        { id: 'original', label: t('original'), hint: t('originalHint') },
        { id: 'ios', label: t('ios'), hint: t('iosHint') },
      ] as Array<{ id: UiEffect; label: string; hint: string }>).map(option => <button key={option.id} type="button" className={`promax-effect-cube ${current === option.id ? 'promax-effect-selected' : ''}`} aria-pressed={current === option.id} disabled={busy} onClick={() => { void choose(option.id) }}>
        <span className={`promax-effect-preview ${option.id === 'original' ? 'promax-effect-preview-original' : ''}`} aria-hidden />
        <span className="promax-effect-name">{option.label}</span>
        <span className="promax-effect-hint">{option.hint}</span>
      </button>)}
    </div>
...
export function applyUiEffect(root: { dataset: DOMStringMap }, effect: UiEffect): void {
  if (effect === 'original') {
    delete root.dataset['dshUiEffect']
  } else {
